...
class AmlResItem(object):
    ...
    def unpack_from(cls, fp):
        ...
        while remaining > 0:
             ...
             if remaining < bite:
			      bite = remaining
             chunk = fp.read(bite)    # Get available data
             h.data.extend(chunk)            # Add to message
             remaining -= len(chunk)	 
 		
        print "Read %d of possible %d" % (len(h.data), h.size)
...
def unpack_image_file(logo_img_file):
    ...
    with open(logo_img_file, "rb") as fp:
        img = AmlResourcesImage.unpack_from(fp)
        for item in img.items:
            print "  Unpacking %s size %d, start %d, datasize %d" % (item.name, item.size, item.start, len(item.data))
            with open("%s.bmp" % item.name, "wb") as item_fp:
